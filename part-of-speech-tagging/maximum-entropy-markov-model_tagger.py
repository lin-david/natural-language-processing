...
def viterbi_decode(Y_pred):
    ...
    backpointers = np.zeros((M, N))

    for s in range(0, L):
        viterbi[s, 0] = math.log(Y_pred[0, s, 1])
        backpointers[s, 0] = 0

    for t in range(1, N):
        for s in range(0, L):
            maxi = float('-inf')
            for s_prime in range(0, L):
                temp = viterbi[s_prime, t - 1] + math.log(Y_pred[t, s_prime, s])
                if (temp >= maxi):
                    maxi = temp
            viterbi[s, t] = maxi
            ...
            for s_prime in range(0, L):
                ...
                if (temp >= maxi):
                    maxi = temp
                    argmaxi = s_prime
            backpointers[s, t] = argmaxi

    # maxi = float('-inf')
    # argmaxi = None
    # for s in range(0, L):
    #     temp = viterbi[s, N-1] * Y_pred[N-1, s, L-1]
    #     if (temp >= maxi):
    #         maxi = temp
    #         argmaxi = s
    # viterbi[L-1, N] = maxi
    # backpointers[L-1, N] = argmaxi

    a = L
    b = N-1
    while (b != 0):
        a = backpointers[a, b].astype(np.int64)
        path.append(a)
        b -= 1
