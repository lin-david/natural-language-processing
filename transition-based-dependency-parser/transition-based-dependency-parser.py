...
def is_projective(toks):
	...
	for i in range(len(toks)):
		...
		# skip root
		if head != 0:
			ranges.append((idd, head))

	# check if arcs have a crossing edge
	for i in range(len(toks)):
		idd = toks[i][0]
		head = toks[i][3]
		...
		for j in range(len(ranges)):
			...
			if arc_head > arc_tail:
				arc_head, arc_tail = arc_tail, arc_head

			if (head < arc_head or head > arc_tail) and idd > arc_head and idd < arc_tail:
				return False
...
def perform_arc(direction, dep_label,
	...
	# as we want to capture the configurations and transition rules
	# before making changes to the stack, wbuffer and arcs
	action = direction + "ARC_" + dep_label
...
def tree_to_actions(wbuffer, stack, arcs, deps):
	...
	while (stack):
		# print(wbuffer)
		if len(stack) <= 1 or stack[-1] == 0 or stack[-2] == 0:
			if not wbuffer:
				dep_label = deps[stack[-2]][(stack[-2], stack[-1])]
		...
		elif stack[-1] in deps.keys() and (stack[-1], stack[-2]) in deps[stack[-1]]:
			dep_label = deps[stack[-1]][(stack[-1], stack[-2])]
			deps[stack[-1]].pop((stack[-1], stack[-2]))
			perform_arc("LEFT", dep_label, wbuffer, stack, arcs, configurations, gold_transitions)
		elif stack[-2] in deps.keys() and (stack[-2], stack[-1]) in deps[stack[-2]]:
			if stack[-1] not in deps.keys() or len(deps[stack[-1]]) == 0:
				dep_label = deps[stack[-2]][(stack[-2], stack[-1])]
				deps[stack[-2]].pop((stack[-2], stack[-1]))
				perform_arc("RIGHT", dep_label, wbuffer, stack, arcs, configurations, gold_transitions)
			else:
				perform_shift(wbuffer, stack, arcs, configurations, gold_transitions)
		else:
...
def action_to_tree(tree, predictions, wbuffer, stack, arcs):
	...
	arg_preds = np.flip(np.argsort(predictions), 1)[0]

	for i in arg_preds:
		action = reverse_labels[i]
